...
COVID_DATA_DIR = os.path.abspath(os.path.join(os.path.abspath('setup.py'), '../data/covid_thermal_dataset'))
COVID_DATA_CACHE = dict()
...
def load_class_images(d):
    if d['class'] not in COVID_DATA_CACHE:
        alphabet, rot = d['class'].split('/')
        image_dir = os.path.join(COVID_DATA_DIR, 'data', alphabet)
        ...
        for sample in loader:
            COVID_DATA_CACHE[d['class']] = sample['data']
            break  # only need one sample because batch size equal to dataset length

    return {'class': d['class'], 'data': COVID_DATA_CACHE[d['class']]}
...
def load(opt, splits):
    split_dir = os.path.join(COVID_DATA_DIR, 'splits', opt['data.split'])
